...
if ("source_to_concept" %in% names(data1)) {
  ...
  if ("source_to_concept" %in% names(data2)) {
    tbl_stc <- tbl_stc %>% 
      full_join(
      ...
      select(source_code, source_description, target_id_new, target_voc_new, 
             target_id_old, target_voc_old
      ) %>%
      mutate(issue = 
    ...
    tbl_stc <- tbl_stc %>% 
      mutate(issue = "removed")
    
  }
}else if("source_to_concept" %in% names(data2)){
  tbl_stc <- data2$source_to_concept %>% 
    select(
      source_code,
      target_voc_new= target_vocabulary_id,
      target_id_new = target_concept_id
