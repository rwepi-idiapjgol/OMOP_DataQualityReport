...
if(runCheckDB1){
  tryCatch({
    ...
    message("Error in getChecksData for DB1: ", e$message)
    csv_files1 <<- c()
  })
}
...
if(runCheckDB2){
  tryCatch({
    ...
    message("Error in getChecksData for DB2: ", e$message)
    csv_files2 <<- c()
  })
}
```
...
if ("procedures" %in% all_tables) {
  data1$procedures %>%
    ...
    complete(vocabulary_id, db, fill = list(n_total_voc = 0)) |> 
    ggplot(aes(x=vocabulary_id, y=n_total_voc, color=db, fill=db))+
    geom_bar(stat="identity", position = "dodge")+
...
if ("drug_quantity" %in% all_tables) {
  data1$drug_quantity %>% 
    ...
    scale_fill_brewer(palette = "Set2", na.value = "lightpink") +  # Paleta automática para fill
...
if ("drug_source_values_id0_n" %in% all_tables & "drugs_id_n" %in% all_tables) {
  ...
  if("drug_source_values_id0_n" %in% names(data1)){
    data1$drug_source_values_id0_n <- data1$drug_source_values_id0_n |> 
      mutate(drug_source_value = as.character(drug_source_value)) 
  }
  if("drug_source_values_id0_n" %in% names(data2)){
    data2$drug_source_values_id0_n <- data2$drug_source_values_id0_n |> 
      mutate(drug_source_value = as.character(drug_source_value)) 
  }
...
```{r,  fig.width=8, fig.height=5, echo=FALSE}
if ("conditions_medications" %in% all_tables) {
  data_cond2 <-data1$conditions_medications%>%
    bind_rows(data2$conditions_medications)%>%
    filter((grepl('Pregnancy',cohort_name)) | (grepl('perinatal',cohort_name)))
  
  if(nrow(data_cond2)!=0){
    data_cond2 |> 
      mutate(cohort_name=swr(cohort_name))%>%
      ggplot(aes(x=cohort_start_date, y=n, color= db, fill=db))+
      facet_grid(cohort_name~ ., scales="free_y")+
...
```{r,  fig.width=8, fig.height=5, echo=FALSE}
if ("conditions_medications" %in% all_tables) {
  data_cond3 <- data1$conditions_medications %>%
    bind_rows(data2$conditions_medications) %>%
    filter((grepl('heart',cohort_name)) | (grepl('veins',cohort_name)) | (grepl('arteries',cohort_name)))
  
 if(nrow(data_cond3)!=0){
    data_cond3 |> 
      mutate(cohort_name=swr(cohort_name))%>%
      ggplot(aes(x=cohort_start_date, y=n, color= db, fill=db))+
      facet_grid(cohort_name~ ., scales="free_y")+
...
```{r,  fig.width=8, fig.height=5, echo=FALSE}
if ("conditions_medications" %in% all_tables) {
  data_cond4 <- data1$conditions_medications%>%
    bind_rows(data2$conditions_medications)%>%
    filter((grepl('nervous',cohort_name)) | (grepl('renal',cohort_name)))
    
  if(nrow(data_cond4)!=0){
    data_cond4 |> 
      mutate(cohort_name=swr(cohort_name))%>%
      ggplot(aes(x=cohort_start_date, y=n, color= db, fill=db))+
      facet_grid(cohort_name~ ., scales="free_y")+
