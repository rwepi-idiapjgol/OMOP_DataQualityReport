...
database1 = "omop_v1"    #old version
database2 = "omop_v2"    #new version
...
if(dir.exists(path1)){
  ...
  message("RunCode.R for database1 should be previously executed.")
}
...
if(dir.exists(path2)){
  ...
  message("RunCode.R for database2 should be previously executed.")
}
...
if ("obs_nationality" %in% names(data1)) {
  data1$obs_nationality %>%
    ...
    arrange(desc(n_new)) %>%
    mutate(
      n_old = ifelse(n_old <5, "<5", as.character(n_old)),
      n_new = ifelse(n_new <5, "<5", as.character(n_new))              
    ) %>% 
...
if ("source_to_concept" %in% names(data1)) {
  data1$source_to_concept %>% 
    select(
      ...
      target_voc_old= target_vocabulary_id, 
      target_id_old = target_concept_id
    ) %>% 
    full_join(
      data2$source_to_concept %>% 
        select(
          source_code,
          target_voc_new= target_vocabulary_id,
          target_id_new = target_concept_id
        ), 
      by ="source_code"
    ) %>% 
    filter(target_id_old != target_id_new) %>% 
    select(source_code, source_description, target_id_new, target_voc_new, target_id_old, target_voc_old) %>% 
    mutate(issue = 
       ifelse(target_voc_new =="None" & target_voc_old !="None", "added", 
              ifelse(target_voc_new !="None" & target_voc_old =="None", "removed","changed")
              )
    ) %>% 
